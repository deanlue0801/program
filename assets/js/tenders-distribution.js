...
function initDistributionPage() {
    ...
    function onQuantityChange(inputElement) {
        ...
        if (currentDistributed > totalQuantity) {
            ...
            // 為了不頻繁打擾，可以選擇不跳 alert，或只在用戶失去焦點時提示
        } else {
    ...
    function showLoading(isLoading, message='載入中...') { const loadingEl = document.querySelector('.loading-overlay'); if(loadingEl) { loadingEl.style.display = isLoading ? 'flex' : 'none'; const textEl = loadingEl.querySelector('p'); if (textEl) textEl.textContent = message; } }
