/**
 * 樓層分配管理系統 (distribution.js) (SPA 版本) - v7.2 (表格欄位最終修正版)
 */
function initDistributionPage() {
    ...
    function buildDistributionTable() {
        ...
        // 步驟 1: 修改表頭，將「項次」和「細項名稱」分開
        let headerHTML = '<tr><th style="width: 80px;">項次</th><th style="width: 250px;">細項名稱</th><th class="total-column">總量</th>';
        ...
        if (detailItems.length === 0) {
            ...
            detailItems.forEach((item, index) => {
                ...
                rowHTML += `<td><div class="item-info"><div class="item-name">${item.name || '未命名'}</div><div class="item-details">單位: ${item.unit || '-'} | 單價: ${formatCurrency(item.unitPrice || 0)}</div></div></td>`;
    ...
    // --- 其他所有函式維持不變 ---
    async function saveAllDistributions() { if (!selectedMajorItem) return showAlert('請先選擇大項目', 'warning'); showLoading(true, '儲存中...'); try { const batch = db.batch(); const existingDistributions = await safeFirestoreQuery("distributionTable", [{ field: "majorItemId", operator: "==", value: selectedMajorItem.id }, { field: "projectId", operator: "==", value: selectedProject.id }]); existingDistributions.docs.forEach(doc => { const docRef = db.collection("distributionTable").doc(doc.id); batch.delete(docRef); }); document.querySelectorAll('.quantity-input').forEach(input => { const quantity = parseInt(input.value) || 0; if (quantity > 0) { const docRef = db.collection("distributionTable").doc(); batch.set(docRef, { projectId: selectedProject.id, tenderId: selectedTender.id, majorItemId: selectedMajorItem.id, detailItemId: input.dataset.itemId, areaType: "樓層", areaName: input.dataset.floor, quantity: quantity, createdBy: auth.currentUser.email, createdAt: firebase.firestore.FieldValue.serverTimestamp() }); } }); await batch.commit(); await loadDistributions(selectedMajorItem.id); buildDistributionTable(); showAlert('✅ 所有分配已儲存成功！', 'success'); } catch (error) { showAlert('儲存失敗: ' + error.message, 'error'); } finally { showLoading(false); } }
    function clearAllDistributions() { if (!selectedMajorItem) return showAlert('請先選擇大項目', 'warning'); if (!confirm(`確定要清空「${selectedMajorItem.name}」的所有樓層分配嗎？\n此操作不會立即儲存，您需要點擊「儲存所有分配」按鈕來確認變更。`)) return; document.querySelectorAll('.quantity-input').forEach(input => { input.value = ''; }); document.querySelectorAll('.item-row').forEach(row => { const distributedCell = document.getElementById(`distributed-${row.dataset.itemId}`); if (distributedCell) { distributedCell.querySelector('strong').textContent = '0'; distributedCell.classList.remove('error'); } }); showAlert('已清空畫面上的分配，請點擊儲存按鈕以生效。', 'info'); }
    function handleFileImport(event) { const file = event.target.files[0]; if (!file) return; if (!selectedMajorItem) { showAlert('請先選擇大項目才能匯入', 'warning'); event.target.value = ''; return; } showLoading(true, '解析檔案中...'); const reader = new FileReader(); reader.onload = (e) => { try { const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, { type: 'array' }); const worksheet = workbook.Sheets[workbook.SheetNames[0]]; const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 }); processImportData(jsonData); } catch (error) { console.error("匯入錯誤:", error); showAlert('檔案解析失敗，請確認檔案為標準 Excel 格式 (.xlsx)', 'error'); } finally { showLoading(false); event.target.value = ''; } }; reader.readAsArrayBuffer(file); }
    function processImportData(data) { if (data.length < 2) return showAlert('匯入檔案中沒有資料', 'warning'); const headers = data[0].map(h => String(h).trim()); const itemNameHeader = headers[0]; const floorHeaders = headers.slice(1, headers.length - 1); let importCount = 0; for (let i = 1; i < data.length; i++) { const row = data[i]; const itemName = String(row[0] || '').trim(); if (!itemName) continue; const detailItem = detailItems.find(item => `${item.sequence || ''} ${item.name}`.trim() === itemName); if (detailItem) { floorHeaders.forEach((floor, index) => { const quantity = parseInt(row[index + 1]) || 0; const input = document.querySelector(`input[data-item-id="${detailItem.id}"][data-floor="${floor}"]`); if (input) { input.value = quantity > 0 ? quantity : ''; if(quantity > 0) importCount++; } }); const firstInput = document.querySelector(`input[data-item-id="${detailItem.id}"]`); if(firstInput) onQuantityChange(firstInput); } } showAlert(importCount > 0 ? `成功匯入 ${importCount} 筆分配資料！` : '沒有找到可匹配的資料', importCount > 0 ? 'success' : 'warning'); }
    function exportToExcel() { if (!selectedMajorItem || detailItems.length === 0) return showAlert('沒有可匯出的資料', 'warning'); const data = []; const headers = ['細項名稱', ...floors, '已分配']; data.push(headers); detailItems.forEach(item => { const row = []; row.push(`${item.sequence || ''} ${item.name}`.trim()); let distributed = 0; floors.forEach(floor => { const input = document.querySelector(`input[data-item-id="${item.id}"][data-floor="${floor}"]`); const quantity = parseInt(input?.value) || 0; row.push(quantity); distributed += quantity; }); row.push(distributed); data.push(row); }); const ws = XLSX.utils.aoa_to_sheet(data); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "樓層分配表"); XLSX.writeFile(wb, `樓層分配_${selectedTender.name}_${selectedMajorItem.name}.xlsx`); }
    function showFloorManager() { if (!selectedTender) return showAlert('請先選擇標單', 'warning'); displayCurrentFloors(); openModal('floorModal'); }
    function displayCurrentFloors() { const container = document.getElementById('currentFloorsList'); if (!container) return; container.innerHTML = floors.length === 0 ? '<p class="empty-modal-text">尚未設定樓層</p>' : floors.map(floor => `<div class="floor-tag" data-floor="${floor}"><span>${floor}</span><button class="remove-floor-btn" data-floor="${floor}">&times;</button></div>`).join(''); container.querySelectorAll('.remove-floor-btn').forEach(btn => btn.onclick = () => { floors = floors.filter(f => f !== btn.dataset.floor); displayCurrentFloors(); }); if (sortableFloor) sortableFloor.destroy(); sortableFloor = new Sortable(container, { animation: 150, onEnd: (evt) => { const element = floors.splice(evt.oldIndex, 1)[0]; floors.splice(evt.newIndex, 0, element); } }); }
    ...
    async function saveFloorSettings() { if (!selectedTender) return showAlert('請先選擇標單', 'warning'); showLoading(true, '儲存樓層設定中...'); try { const floorData = { projectId: selectedProject.id, tenderId: selectedTender.id, floors: floors, updatedAt: firebase.firestore.FieldValue.serverTimestamp(), updatedBy: auth.currentUser.email }; const existingResult = await safeFirestoreQuery("floorSettings", [{ field: "tenderId", operator: "==", value: selectedTender.id }, { field: "projectId", operator: "==", value: selectedProject.id }]); if (existingResult.docs.length > 0) { const docId = existingResult.docs[0].id; await db.collection("floorSettings").doc(docId).update(floorData); } else { floorData.createdAt = firebase.firestore.FieldValue.serverTimestamp(); floorData.createdBy = auth.currentUser.email; await db.collection("floorSettings").add(floorData); } closeModal('floorModal'); if (selectedMajorItem) await loadMajorItemData(selectedMajorItem.id); showAlert('✅ 樓層設定已成功儲存！', 'success'); } catch (error) { showAlert('儲存樓層設定失敗: ' + error.message, 'error'); } finally { showLoading(false); } }
    function showSequenceManager() { if (!selectedMajorItem) return showAlert('請先選擇大項目', 'warning'); const list = document.getElementById('sequenceList'); if (!list) return; list.innerHTML = ''; detailItems.forEach(item => { const div = document.createElement('div'); div.className = 'sequence-item'; div.dataset.itemId = item.id; div.innerHTML = `<span class="drag-handle">☰</span><input class="sequence-input" value="${item.sequence || ''}" placeholder="項次"><span class="item-name">${item.name}</span>`; list.appendChild(div); }); if (sortableSequence) sortableSequence.destroy(); sortableSequence = new Sortable(list, { handle: '.drag-handle', animation: 150 }); openModal('sequenceModal'); }
    async function saveSequenceChanges() { const items = document.querySelectorAll('#sequenceList .sequence-item'); if (items.length === 0) return closeModal('sequenceModal'); showLoading(true, '儲存順序中...'); try { const batch = db.batch(); const newDetailItems = []; items.forEach((item) => { const itemId = item.dataset.itemId; const sequence = item.querySelector('.sequence-input').value.trim(); const docRef = db.collection("detailItems").doc(itemId); batch.update(docRef, { sequence }); const originalItem = detailItems.find(d => d.id === itemId); if(originalItem) newDetailItems.push({ ...originalItem, sequence }); }); await batch.commit(); detailItems = newDetailItems.sort(naturalSequenceSort); buildDistributionTable(); closeModal('sequenceModal'); showAlert('項次順序已成功儲存！', 'success'); } catch (error) { showAlert('儲存項次順序失敗: ' + error.message, 'error'); } finally { showLoading(false); } }
    function setupEventListeners() { document.getElementById('projectSelect')?.addEventListener('change', onProjectChange); document.getElementById('tenderSelect')?.addEventListener('change', onTenderChange); document.getElementById('majorItemSelect')?.addEventListener('change', onMajorItemChange); document.getElementById('saveDistributionsBtn')?.addEventListener('click', saveAllDistributions); document.getElementById('clearDistributionsBtn')?.addEventListener('click', clearAllDistributions); document.getElementById('importBtn')?.addEventListener('click', () => document.getElementById('importInput').click()); document.getElementById('importInput')?.addEventListener('change', handleFileImport); document.getElementById('exportBtn')?.addEventListener('click', exportToExcel); document.getElementById('floorManagerBtn')?.addEventListener('click', showFloorManager); document.getElementById('sequenceManagerBtn')?.addEventListener('click', showSequenceManager); document.getElementById('templateButtons')?.addEventListener('click', (e) => { if (e.target.tagName === 'BUTTON') applyFloorTemplate(e.target.dataset.template); }); document.getElementById('addCustomFloorBtn')?.addEventListener('click', addCustomFloor); document.getElementById('clearAllFloorsBtn')?.addEventListener('click', clearAllFloors); document.getElementById('saveFloorSettingsBtn')?.addEventListener('click', saveFloorSettings); document.getElementById('cancelFloorModalBtn')?.addEventListener('click', () => closeModal('floorModal')); document.getElementById('saveSequenceBtn')?.addEventListener('click', saveSequenceChanges); document.getElementById('cancelSequenceModalBtn')?.addEventListener('click', () => closeModal('sequenceModal')); }
