/**
 * 標單詳情頁面 (tenders/detail.js) - 對應新版UI並增加追蹤開關
 */
function initTenderDetailPage() {
    ...
    function createMajorItemCard(majorItem) {
        ...
        // **修改點**：按鈕 onclick 事件改為呼叫 window.exposedFunctions 中的函式
        majorItemDiv.innerHTML = `
            <div class="major-item-header">
                ...
                <div class="major-item-actions">
                    ...
                    <button class="btn btn-secondary" onclick="window.exposedFunctions.toggleMajorItemSummary('${majorItem.id}')">👁️ 預覽</button>
    ...
    async function handleProgressTrackingToggle(event) {
        ...
        try {
            ...
            toggle.checked = !exclude; // 失敗時還原開關狀態
        } finally {
    ...
    window.exposedFunctions = {
        ...
        toggleDetailItemsTable // **新增**暴露此函數
    };
