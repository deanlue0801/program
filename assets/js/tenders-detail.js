...
function initTenderDetailPage() {
    ...
    let currentTender = null, currentProject = null, currentUserPermissions = {}, currentUserRole = null;
    let majorItems = [], detailItems = [], distributionData = [], tenderId = null, allMajorExpanded = false;
    ...
    async function loadProjectAndCheckPermissions() {
        if (!currentTender.projectId) { currentProject = null; return false; }
        try {
            const projectDoc = await db.collection('projects').doc(currentTender.projectId).get();
            if (!projectDoc.exists) { currentProject = null; return false; }
            currentProject = { id: projectDoc.id, ...projectDoc.data() };

            const userEmail = auth.currentUser.email;
            if (!currentProject.memberEmails.includes(userEmail)) return false;
