...
function initTenderDetailPage() {
    ...
    function createDetailItemsTable(details) { 
        ...
        table.innerHTML = `<thead><tr><th style="width: 5%;">項次</th><th style="width: 40%;">項目名稱</th><th style="width: 5%;">單位</th><th style="width: 10%;">數量</th><th style="width: 15%;">單價</th><th style="width: 15%;">總價</th><th style="width: 10%;">進度追蹤</th></tr></thead><tbody></tbody>`; 
        ...
        if (details.length === 0) { 
            ...
            details.forEach(item => { 
                ...
                row.innerHTML = `<td>${item.sequence || ''}</td><td>${item.name || ''}</td><td>${item.unit || ''}</td><td>${quantity}</td><td>${unitPrice.toLocaleString()}</td><td>${(quantity * unitPrice).toLocaleString()}</td><td><label class="toggle-switch"><input type="checkbox" class="progress-tracking-toggle"><span class="slider"></span></label></td>`; 
