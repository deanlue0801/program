...
function initProgressManagementPage() {
    ...
    // deanlue0801/program/program-efeb15ebc0ff815486868107b565e4f2cf1935ae/assets/js/tenders-progress-management.js

async function onStatusChange(selectElement) {
    const canEditStatus = currentUserRole === 'owner' || (currentUserRole === 'editor' && currentUserPermissions.canAccessTenders);
    if (!canEditStatus) {
        showAlert('權限不足，無法修改狀態', 'error');
        selectElement.value = selectElement.dataset.previousValue || '未施工';
        return;
    }
    ...
    try {
        // 【核心修正】在這裡的查詢中，加上 projectId 的條件
        const querySnapshot = await safeFirestoreQuery("progressItems", [
            { field: "uniqueId", operator: "==", value: uniqueId },
            { field: "projectId", operator: "==", value: selectedProject.id } // <-- 新增這一行
        ]);
        ...
        // 現在這裡應該不會再報錯，但保留錯誤處理
        console.error("[ERROR] 儲存狀態失敗:", error);
