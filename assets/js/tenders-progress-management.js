/**
 * 施工進度管理 (progress-management.js) - v4.1 (最終修正版)
 */
function initProgressManagementPage() {
    ...
    async function onTenderChange(tenderId) {
        ...
        try {
            ...
            if(workItemsDoc.docs.length > 0 && workItemsDoc.docs[0].items) {
                workItems = workItemsDoc.docs[0].items;
            }
            populateSelect(document.getElementById('majorItemSelect'), majorItems, '請選擇大項目...');
        } catch (error) {
            showAlert('載入大項目或設定失敗: ' + error.message, 'error');
        }
    }
    ...
    async function onFloorChange(floorName) {
        ...
        try {
            ...
            populateSelect(spaceSelect, spaces.map(s => ({id:s, name:s})), '篩選空間 (可選)');
            ...
            await loadProgressData();
             ...
             showAlert('載入空間資料失敗: ' + error.message, 'error');
        }
    }
