/**
 * 編輯標單頁面 (tenders-edit.js) - v5.2 修正版
 */
function initTenderEditPage() {
    ...
    async function loadAllData() {
        try {
            ...
            // 修正：正確存取 Firestore 文檔資料
            majorItems = majorItemsData.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort(naturalSequenceSort);
            
            const allDetailItems = allDetailItemsData.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            detailItems = allDetailItems.filter(item => !item.isAddition).sort(naturalSequenceSort);
            additionItems = allDetailItems.filter(item => item.isAddition).sort((a, b) => 
    ...
    function updateSingleDetailItemRow(detailItemId) {
        const row = document.querySelector(`tr[data-detail-item-id="${detailItemId}"]`);
        const item = detailItems.find(d => d.id === detailItemId);
        
        // 修正：增加安全性檢查
        if (!row || !item) {
            console.warn(`找不到要更新的行或項目: ${detailItemId}`);
            return;
        }
        ...
        const cell = row.querySelector('.current-quantity-cell');
        if (cell) {
            cell.innerHTML = quantityDisplay;
        }
    }
    ...
    function renderDetailTable(details) {
            ...
            return `
                <tr data-detail-item-id="${item.id}">
                    ...
                    <td class="action-cell">
                        <button class="btn btn-sm btn-primary" data-action="add-addition" data-item-id="${item.id}">追加</button>
                    </td>
                </tr>
            `;
        }).join('');
        ...
        return `
            <table class="detail-items-table">
                <thead>
                    <tr>
                        <th>項次</th><th>項目名稱</th><th>單位</th><th class="number-cell">數量</th>
                        <th class="number-cell">單價</th><th class="number-cell">小計</th>
                        <th class="number-cell">目前總數</th><th class="action-cell">操作</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
        ...
        container.innerHTML = `
            <div class="tender-header-card">
                <div class="tender-info-item">
                    ...
                    <div class="value large">${escapeHtml(currentTender.name || '')}</div>
                </div>
                <div class="tender-info-item">
    ...
    function renderMajorItemsList() {
        ...
        majorItems.forEach((majorItem) => {
            ...
            const totalPrice = detailsInMajor.reduce((s, i) => s + (i.totalPrice || 0), 0);
            
            html += `
                <div class="major-item-wrapper">
                    <div class="major-item-row ${allExpanded ? 'expanded' : ''}" data-major-id="${majorItem.id}">
                        <div class="item-number-circle">${escapeHtml(itemNumber)}</div>
                        <div class="item-name">${escapeHtml(majorItem.name || '')}</div>
    ...
    function renderAdditionItemsTable() {
        ...
        const rows = additionItems.map(add => {
            ...
            const displayDate = add.additionDate || (add.createdAt ? formatDate(add.createdAt) : '');
            
            return `
                <tr>
                    <td>${escapeHtml(displayDate)}</td>
                    <td>${escapeHtml(relatedItem ? relatedItem.name : '未知項目')}</td>
                    <td class="number-cell">${add.totalQuantity || 0}</td>
                    <td class="number-cell">${formatCurrency(add.unitPrice)}</td>
                    <td>${escapeHtml(add.reason || '')}</td>
                    <td>${escapeHtml(add.notes || '')}</td>
                    <td class="action-cell">
        ...
        html += `
            <table class="detail-items-table">
                <thead>
                    <tr>
                        <th>追加日期</th><th>關聯項目</th><th class="number-cell">追加數量</th>
                        <th class="number-cell">追加單價</th><th>追加原因</th><th>備註</th><th class="action-cell">操作</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
    ...
    function renderSummaryCards() {
        ...
        summaryContainer.innerHTML = Object.values(summarizedItems).map(summary => `
            <div class="summary-card">
                <div class="summary-item-name">${escapeHtml(summary.name)}</div>
                <div class="summary-detail">
                    <span>原始數量</span>
                    <span>${summary.originalQty} ${escapeHtml(summary.unit || '')}</span>
                </div>
                <div class="summary-detail">
                    <span>追加數量</span>
                    <span class="addition">+${summary.additionalQty} ${escapeHtml(summary.unit || '')}</span>
                </div>
                <div class="summary-detail total">
                    <span>目前總數</span>
                    <span>${summary.originalQty + summary.additionalQty} ${escapeHtml(summary.unit || '')}</span>
                </div>
            </div>
        `).join('');
    }
    ...
    function setupEventListeners() {
        ...
        content.addEventListener('click', (event) => {
            ...
            if (majorRow) {
                ...
                const detailContainer = majorRow.nextElementSibling;
                if (detailContainer) {
            ...
            if (expandAllBtn) {
                ...
                document.querySelectorAll('.major-item-row').forEach(row => {
                    row.classList.toggle('expanded', allExpanded);
                });
                document.querySelectorAll('.detail-items-container').forEach(cont => {
                    cont.classList.toggle('expanded', allExpanded);
                });
            ...
            if (editId) {
                ...
                if (index > -1) {
                    additionItems[index] = { 
                        ...additionItems[index], 
                        ...data,
                        updatedAt: new Date() // 使用本地時間避免 null 值
                    };
                }
            } else {
                data.createdBy = currentUser?.email || 'unknown';
                data.createdAt = firebase.firestore.FieldValue.serverTimestamp();
                const docRef = await db.collection('detailItems').add(data);
                // 修正：新增本地資料時處理時間戳
                additionItems.unshift({ 
                    id: docRef.id, 
                    ...data,
                    createdAt: new Date(),
                    updatedAt: new Date()
    ...
    function showAdditionModal(relatedItemId = null) {
        ...
        const editIdInput = document.getElementById('editAdditionId');
        if (editIdInput) editIdInput.value = '';
        
        const modalTitle = document.getElementById('modalTitle');
        if (modalTitle) modalTitle.textContent = '新增追加項目';
        ...
        const dateInput = document.getElementById('additionDate');
        if (dateInput) {
    ...
    function editAddition(additionId) {
        ...
        const modalTitle = document.getElementById('modalTitle');
        if (modalTitle) modalTitle.textContent = '編輯追加項目';

        populateRelatedItemsDropdown(item.relatedItemId);
        
        // 填入表單資料
        const fields = [
            { id: 'additionDate', value: item.additionDate || '' },
            { id: 'additionQuantity', value: item.totalQuantity || '' },
            { id: 'additionUnitPrice', value: item.unitPrice || '' },
            { id: 'additionReason', value: item.reason || '' },
            { id: 'additionNotes', value: item.notes || '' }
        ];
